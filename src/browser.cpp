    ...
    , ui(new Ui::Browser)
{
    ...
    layer->get_ui()->te_url->installEventFilter(this);
...
void Browser::create_view(QUrl url, bool is_link)
{
   ...
   // TODO: if (mouse-click)
    ...
    if (!is_link) {
        ui->l_main->insertWidget(0, views.back());
...
void Browser::add_tab(int index, bool rotate, bool is_link)
{
    ...
    if (is_link) {
        ...
        tab = index;
        self = current;
        ui->l_main->removeWidget(views[self]);
    }

    qDebug() << "ADD:" << "TAB" << tab << "SELF" << self;
    layer->get_ui()->l_tabs->addWidget(views[self]);
    views[self]->setMaximumSize(300, 250); // TODO: %
    views[self]->setZoomFactor(0.35);
    views[self]->get_ui()->b_tab->setParent(views[self]);
    ...
    connect(views[self]->get_ui()->b_tab_delete, &QPushButton::released, [=] {
        delete_tab(self);
    });
    ...
    if (layer->get_ui()->w_tabs->isHidden())
        layer->get_ui()->w_tabs->show();
}

void Browser::open_tab(int tab)
{
    layer->get_ui()->l_tabs->removeWidget(views[tab]);
    views[tab]->get_ui()->b_tab->hide();
    views[tab]->get_ui()->b_tab->setParent(NULL);
    views[tab]->setMaximumSize(1920, 1080); // TODO: %
    views[tab]->setZoomFactor(1.0);
    ui->l_main->insertWidget(0, views[tab]);
...
void Browser::delete_tab(int tab)
{
    ...
    for (uint i = tab; i < (views.size() - 1); i++) {
        ...
        connect(views[i]->get_ui()->b_tab_delete, &QPushButton::released, [=] {
            delete_tab(i);
        });
    }

    if (views.size() <= 1)
        layer->get_ui()->w_tabs->hide();
}
    ...
    if (e->type() == QEvent::KeyPress) {
        ...
        if (o == this) {
            ...
            if (key->modifiers() == Qt::MetaModifier)
                ...
                is_link_to_tab = true;
        } else if (o == layer->get_ui()->te_url) {
            switch(key->key()) {
                case Qt::Key_Return:
                    if (!layer->get_ui()->te_url->document()->toPlainText().simplified().isEmpty()) {
                        url.setUrl(layer->get_ui()->te_url->document()->toPlainText().simplified());
